...
public class QuotesServiceImpl implements QuotesService {
    ...
    public Quotes update(Quotes quotes) {
        ...
        List<Sales> saleses = quotes.getSaleses();
        List<FollowUps> followUpses = quotes.getFollowUpses();
        if(saleses != null && Hibernate.isInitialized(saleses)) {
            saleses.forEach(_sales -> _sales.setQuotes(quotes));
        }
        if(followUpses != null && Hibernate.isInitialized(followUpses)) {
            followUpses.forEach(_followUps -> _followUps.setQuotes(quotes));
        }
    ...
    public void export(ExportOptions options, Pageable pageable, OutputStream outputStream) {
        LOGGER.debug("exporting data in the service salesdb for table Quotes to {} format", options.getExportType());
